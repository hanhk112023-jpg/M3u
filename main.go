...
func main() {
	...
	flag.StringVar(&cfg.ChannelLogo, "tvg-logo", "https://raw.githubusercontent.com/hanhk112023-jpg/M3u/main/logo.png",
		"channel logo URL used for tvg-logo/group-title; overrides the per-room cover image")
	flag.Parse()
	...
	scan := func() error {
		...
		var writeErr error
		if cfg.Format == "json" {
			writeErr = writeIPTVJSON(cfg.Out, result.Channels, cfg.ChannelLogo)
		} else {
			writeErr = writePlaylist(cfg.Out, result.Channels, cfg.EPG, cfg.ChannelLogo)
		}
		if writeErr != nil {
		...
		if cfg.Publish {
			newItems := iptvJSONItems(result.Channels, cfg.ChannelLogo)
			...
			jsonData, err := iptvJSONBytes(result.Channels, cfg.ChannelLogo)
			if err != nil {
			...
			m3uData, err := m3uBytes(result.Channels, cfg.EPG, cfg.ChannelLogo, time.Now())
			if err != nil {
...
// When channelLogo is non-empty it replaces the per-room cover image so
// players show a uniform channel mark instead of match thumbnails.
func m3uBytes(channels []Channel, epg string, channelLogo string, now time.Time) ([]byte, error) {
...
func writePlaylist(path string, channels []Channel, epg, channelLogo string) error {
	data, err := m3uBytes(channels, epg, channelLogo, time.Now())
	if err != nil {
		return err
	}
	if path == "-" {
...
// output and the change detector. channelLogo, when non-empty, overrides the
// per-room cover so the published playlist stays consistent with the M3U.
func iptvJSONItems(channels []Channel, channelLogo string) []IPTVJSONChannel {
...
func iptvJSONBytes(channels []Channel, channelLogo string) ([]byte, error) {
	data, err := json.MarshalIndent(iptvJSONItems(channels, channelLogo), "", "  ")
	if err != nil {
		return nil, err
	}
	return append(data, '\n'), nil
...
func writeIPTVJSON(path string, channels []Channel, channelLogo string) error {
	data, err := iptvJSONBytes(channels, channelLogo)
	if err != nil {
		return err
	}
	if path == "-" {
...
func handlePlaylist(w http.ResponseWriter, r *http.Request, cfg Config, store *ChannelStore) {
	...
	data, err := m3uBytes(channels, cfg.EPG, cfg.ChannelLogo, time.Now())
	if err != nil {
